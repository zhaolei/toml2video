...
def generate_video_from_toml(config_path):
    ...
    bgm_file = config.get("bgm", None)  # 读取背景音乐
    ...
    with open(file_list_path, "w", encoding="utf-8") as fl, open(subtitle_path, "w", encoding="utf-8") as srt:
        ...
        for item in items:
            ...
            total_duration = end_time  # 记录视频总时长
    ...
    if bgm_file and os.path.exists(bgm_file):
        audio_stream = ffmpeg.input(bgm_file)
        audio_stream = ffmpeg.filter_(audio_stream, "atrim", duration=total_duration)  # 裁剪音乐到视频长度
        audio_stream = ffmpeg.filter_(audio_stream, "volume", 0.8)  # 调低音量
        output_stream = ffmpeg.output(input_stream, audio_stream, output_video, vcodec="libx264", acodec="aac", pix_fmt="yuv420p")
    else:
